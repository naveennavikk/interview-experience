...
if(isset($_SESSION['email']))
{
    if(isset($_GET['aid']) && isset($_GET['uid']) && isset($_GET['upvote']))
    {
        if(is_numeric($_GET['aid']) && is_numeric($_GET['uid']) && is_numeric($_GET['upvote']))
        {
            ...
            if($upvote)
            {
                $checkQuery = "SELECT count(*) FROM answer_upvotes WHERE aid='$aid' AND uid='$uid'";
                ...
                if($checkQuery3[0] == 0)
                {
                   $downQuery = "INSERT INTO answer_upvotes VALUES('','$uid','$aid','$time')"; // Following.
                   $downQuery2 = mysqli_query($con,$downQuery) or die(mysqli_error($con));

                   $downQuery3 = "SELECT upvotes FROM answers WHERE sno='$aid'";
                   ...
                   $downvotes = $downQuery5['upvotes'];
                   $n = $downvotes + 1;

                   $downQuery6 = "UPDATE answers SET upvotes='$n' WHERE sno='$aid'";
                   $downQuery6 = mysqli_query($con,$downQuery6) or die(mysqli_error($con));


                   $upvote = "SELECT count(*) FROM answer_downvotes WHERE aid='$aid' AND uid='$uid'";
                   $upvote2 = mysqli_query($con,$upvote) or die(mysqli_error($con));
                   $upvote3 = mysqli_fetch_row($upvote2) or die(mysqli_error($con));

                   if($upvote3[0] > 0)
                   {
                        $upvotequery = "DELETE FROM answer_downvotes WHERE aid='$aid' AND uid='$uid'";
                        $upvotequery2 = mysqli_query($con,$upvotequery) or die(mysqli_error($con));

                        $downvoteQuery7 = "SELECT downvotes FROM answers WHERE sno='$aid'";
                        ...
                        $downvotess = $downvoteQuery9['downvotes'];
                        $m = $downvotess - 1;
     
                        $downvoteQuery10 = "UPDATE answers SET downvotes='$m' WHERE sno='$aid'";
            ...
            else
            {
                ...
                if($checkQuery3[0] > 0)
                {
                        $undodownvotequery = "DELETE FROM answer_upvotes WHERE aid='$aid' AND uid='$uid'";
                        $undodownvotequery2 = mysqli_query($con,$undodownvotequery) or die(mysqli_error($con));

                        $downvoteQuery3 = "SELECT upvotes FROM answers WHERE sno='$aid'";
                        ...
                        $downvotes = $downvoteQuery5['upvotes'];
                        $n = $downvotes - 1;
     
                        $downvoteQuery6 = "UPDATE answers SET upvotes='$n' WHERE sno='$aid'";
